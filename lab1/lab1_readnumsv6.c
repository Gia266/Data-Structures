...
//delete integer d
{
    ...
    if (phead != NULL){
        ...
        //when value is found free the memory to delete it
        if (phead-> datavalue == d) {
            phead = phead->pnextnode;
            free(curr);
        }
        else{
            //check is the value to delete in the list or not?

            prev = NULL;
            while (curr != NULL && curr->datavalue != d) {
                prev = curr;
                curr = curr->pnextnode;
            }
            //check was the values found?
            if (curr != NULL) {
                //yay! found it
                prev->pnextnode = curr ->pnextnode;
                free (curr);
            }

            if (curr == phead) {
                phead = curr->pnextnode;
            }
